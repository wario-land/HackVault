...
try{
	...
	webWorkerApply.onmessage = event => { // listen for events from the worker
		...
		if(event.data.patchedRomU8Array)
			preparePatchedRom(romFile,patchFile, new MarcFile(event.data.patchedRomU8Array.buffer), headerSize);
...
function preparePatchedRom(originalRom, patchFile, patchedRom, headerSize){
	patchedRom.fileName=patchFile.fileName.replace(/\.([^\.]*?)$/, '.gba');
	...
	if(p && r){
		...
		if(CAN_USE_WEB_WORKERS){
			...
			try{
				...
				preparePatchedRom(r,event.data.patchFileU8Array, p.apply(r, validateChecksums), headerSize);
