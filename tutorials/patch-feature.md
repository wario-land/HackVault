...
**What if i want to use some functions or some const arrays in multiple C patches but want to maintain them in one place?** You can put them into a saperate C file, and set the patch type to "C_dependency" in the Patch Edit Dialog manually when loading the patch file. An example of "C_dependency" patch file looks like this:
```
...
```
+ Always remember that: don't set the @HookAddress and the @HookString in the "C_dependency" type patch file, or it cannot be loaded into the Patch Manager of WL4Editor. Also in every "C_dependency" type patch file, the @EntryFunctionSymbol will be ignored.

+ How it works: "C_dependency" type patches will always be compiled and linked before other regular patches. By grabbing all the global symbols of "C_dependency" type patches from the generated ".elf.txt", putting them into the linker script for the rest of the regular C patches, All the other regular patches can use global symbols from all "C_dependency" patches.
